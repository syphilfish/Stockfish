...
ExtMove* MovePicker::score(const MoveList<Type>& ml) {
    ...
    for (auto move : ml)
    {
        ...
        else if constexpr (Type == QUIETS)
        {
            ...
            // bonus for checks, tapered by the fifty-move counter: a quiet check
            // cannot reset rule50, so its strategic premium fades as a draw nears.
            m.value += ((pos.check_squares(pt) & to) && pos.see_ge(m, -75))
            * std::max(4096, 16384 - 128 * pos.rule50_count());
